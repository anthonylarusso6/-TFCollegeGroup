...
export default function Draft({athletes}){
  ...
  const[step,setStep]=useState("pool"); // pool → leaders → watching
  ...
  return(
    <div>
      ...
      {step==="pool"&&(
        <div>
          <div style={{background:"#fff",borderRadius:12,padding:"1.25rem",marginBottom:12,border:"0.5px solid #e0e0e0",borderTop:"3px solid "+PUR}}>
            ...
            <div style={{fontSize:12,color:"#888",marginBottom:14}}>Check everyone who's here today. Uncheck anyone who's absent or sitting out. Only checked athletes go into the draft pool.</div>
              ...
              {athletes.map(a=>{
                ...
                return(
                  <button key={a.id} onClick={()=>togglePool(a.name)} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",borderRadius:10,border:"0.5px solid "+(inPool?PUR:"#e0e0e0"),background:inPool?"#EEEDFE":"#f9f9f9",cursor:"pointer",fontFamily:"Georgia,serif",textAlign:"left"}}>
                    ...
                    <div style={{width:32,height:32,borderRadius:"50%",background:a.role==="forge"?RED:STEEL,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:500,color:"#fff",flexShrink:0}}>{a.name[0]}</div>
                    <div style={{flex:1}}>
                      <div style={{fontSize:13,fontWeight:500,color:inPool?"#1a1a1a":"#aaa"}}>{a.name}</div>
                      <div style={{fontSize:11,color:"#888"}}>{a.sport}</div>
                    </div>
                    <div style={{fontSize:11,color:inPool?PUR:"#ccc"}}>{inPool?"In pool":"Out"}</div>
                  </button>
                );
              })}
            </div>
      ...
      {step==="leaders"&&(
        <div>
          <div style={{background:"#fff",borderRadius:12,padding:"1.25rem",marginBottom:12,border:"0.5px solid #e0e0e0",borderTop:"3px solid "+PUR}}>
            <div style={{fontSize:13,fontWeight:600,color:"#1a1a1a",marginBottom:4}}>Step 2 — Confirm leaders</div>
            <div style={{fontSize:12,color:"#888",marginBottom:14}}>System picked 4 random leaders from your pool. Swap any before starting. Once you hit Start Draft — Forge leaders pick bracelets on their phones and the live draft begins.</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              {[0,1,2,3].map(i=>(
                <div key={i} style={{background:LB[i],borderRadius:10,padding:"12px",border:"0.5px solid "+LC[i]+"44"}}>
                  ...
                  <div style={{fontSize:14,fontWeight:500,color:"#1a1a1a",marginBottom:8}}>{leaders[i]||"—"}</div>
                  {swapIdx===i?(
                    ...
                    <button onClick={()=>setSwapIdx(swapIdx===i?null:i)} style={{fontSize:11,padding:"4px 12px",borderRadius:6,border:"0.5px solid "+LC[i],background:"transparent",color:LC[i],cursor:"pointer",fontFamily:"Georgia,serif"}}>Swap</button>
                  )}
                </div>
              ))}
            </div>
      ...
      {step==="watching"&&draft&&(
        <div>
          ...
          {phase==="locked"&&!editMode&&(
            <button onClick={resetDraft} style={{width:"100%",padding:"10px",borderRadius:8,border:"0.5px solid "+PUR,background:"transparent",color:PUR,fontSize:13,fontWeight:500,cursor:"pointer",fontFamily:"Georgia,serif"}}>
