...
export default function MindsetMonday(){
  ...
  const[notes,setNotes]=useState(()=>{
    try{const s=localStorage.getItem("mm_notes");return s?JSON.parse(s):{};}catch{return {};}
  });
  const[completedWeeks,setCompletedWeeks]=useState(()=>{
    try{const s=localStorage.getItem("mm_completed");return s?JSON.parse(s):[];}catch{return [];}
  });

  useEffect(()=>{try{localStorage.setItem("mm_notes",JSON.stringify(notes));}catch{};},[notes]);
  useEffect(()=>{try{localStorage.setItem("mm_completed",JSON.stringify(completedWeeks));}catch{};},[completedWeeks]);
  ...
  return(
    <div>
      ...
      <div style={{background:"#141414",border:"0.5px solid #252525",borderRadius:12,padding:"1rem 1.25rem",marginBottom:12,marginTop:12}}>
        ...
        <textarea value={notes[noteKey]||""} onChange={e=>setNotes(p=>({...p,[noteKey]:e.target.value}))} placeholder="What came up in discussion? What hit? Who opened up? What to follow up on?" rows={5} style={{width:"100%",minHeight:120,padding:"10px",fontSize:13,border:"0.5px solid #333",borderRadius:8,background:"#111",color:"#ddd",fontFamily:"Georgia,serif",resize:"vertical",boxSizing:"border-box",lineHeight:1.6}}/>
      </div>
