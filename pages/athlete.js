...
export default function Athlete(){
  ...
  const loadData=async()=>{
    setLoading(true);
    try{const{data:aths}=await supabase.from("athletes").select("*").eq("status","active").order("name");if(aths)setAthletes(aths);}catch(e){}
    try{const{data:ann}=await supabase.from("announcements").select("*").eq("active",true).order("created_at",{ascending:false}).limit(1);if(ann&&ann.length>0)setAnnouncement(ann[0]);}catch(e){}
    try{const{data:anv}=await supabase.from("anvil").select("*").eq("type","individual").order("created_at",{ascending:false}).limit(1);if(anv&&anv.length>0)setAnvilWinner(anv[0]);}catch(e){}
    setLoading(false);
  };
  ...
  if(screen==="login")return(
    <>
      ...
      <div style={{minHeight:"100vh",background:BG,fontFamily:"Georgia, serif",maxWidth:480,margin:"0 auto",padding:"3rem 1.5rem 2rem",textAlign:"center",position:"relative"}}>
        ...
        {/* Visible text input — tap to type from keyboard */}
        <input
          autoFocus
          type="password"
          inputMode="numeric"
          ...
          onChange={e=>{
            ...
            if(val.length===4){
              setTimeout(()=>{
                // Trigger pin check
                const syntheticKey=parseInt(val[3]);
                const newPin=val;
                if(!selectedAthlete?.pin){
                  if(pinStep==="enter"){setPinStep("confirm");setPinConfirm(newPin);setPin("");}
                  else if(pinStep==="confirm"){
                    if(newPin===pinConfirm){
                      supabase.from("athletes").update({pin:newPin}).eq("id",selectedAthlete.id).then(()=>{
                        setSelectedAthlete(a=>({...a,pin:newPin}));
                        setScreen("checkin");setPin("");setPinStep("enter");
                      });
                    }else{setPinError("PINs don't match. Try again.");setPin("");setPinStep("enter");setPinConfirm("");}
                  }
                }else{
                  if(newPin===selectedAthlete.pin){setScreen("checkin");setPin("");setPinStep("enter");}
        ...
        <div style={{marginTop:12,fontSize:11,color:"#444"}}>Tap the dots to use your keyboard</div>
        {pinError&&<div style={{marginTop:10,fontSize:13,color:"#E24B4A"}}>{pinError}</div>}
      </div>
    </>
  );
