...
function DailyWord({announcement}){
  ...
  const today=new Date();
  const word=WORDS[today.getDate()%WORDS.length];
  const PUR="#534AB7",BG="#0f0f0f";
  return(
    <div style={{background:BG,borderRadius:12,padding:"1.25rem",marginBottom:12,border:"0.5px solid #222",position:"relative",overflow:"hidden"}}>
      ...
      {announcement&&(
        <div style={{fontSize:13,color:"#fff",fontWeight:500,marginBottom:10,padding:"8px 10px",background:"#1a1a1a",borderRadius:8,borderLeft:"3px solid #D4AF37"}}>{announcement}</div>
      )}
...
function ClassCountdown(){
  ...
  return(
    <div style={{background:BG,borderRadius:12,padding:"1.5rem",marginBottom:12,border:"1px solid "+GOLD+"44",textAlign:"center",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{fontSize:12,color:"#444"}}>June 18, 2025 · 9:00am · Iron sharpens iron</div>
    </div>
  );
}
...
function GroupStreak({attendance,athletes}){
  ...
  return(
    <div style={{background:allEarly?"linear-gradient(135deg,#0d1f0f,#1a3a1f)":BG,borderRadius:12,padding:"1rem 1.25rem",marginBottom:12,border:"1px solid "+(allEarly?GREEN+"66":"#222")}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontSize:11,color:allEarly?GREEN:"#555",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:4}}>{allEarly?"🔥 Squad showed up!":"Last class check-in"}</div>
          <div style={{fontSize:14,fontWeight:600,color:"#fff"}}>{earlyCount}/{totalActive} athletes early {allEarly?"— whole squad! 🔥":""}</div>
        </div>
        <div style={{fontSize:28,fontWeight:800,color:allEarly?GREEN:"#333"}}>{pct}%</div>
      </div>
      {allEarly&&<div style={{fontSize:12,color:GREEN,marginTop:6,fontStyle:"italic"}}>"Iron sharpens iron — the whole squad answered the call."</div>}
    </div>
  );
}
...
function PRLog({athleteId}){
  ...
  const[saving,setSaving]=useState(false);
  const[saved,setSaved]=useState(false);
  const GREEN="#1E6B3A",RED="#C0392B",GOLD="#D4AF37",BG="#0f0f0f";
  ...
  const bests={};
  prs.forEach(p=>{
    if(!bests[p.lift]||p.weight>bests[p.lift].weight)bests[p.lift]=p;
  });
    ...
    <div>
      ...
      {Object.keys(bests).length>0&&(
        <div style={{background:"#fff",borderRadius:12,padding:"1.25rem",marginBottom:12,border:"0.5px solid #e0e0e0"}}>
          ...
          {Object.entries(bests).map(([lift,pr],i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<Object.keys(bests).length-1?"0.5px solid #f0f0f0":"none"}}>
              <div style={{fontSize:13,color:"#1a1a1a"}}>{lift}</div>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
...
// ── Achievement Badges ──────────────────────────────────────────
function AchievementBadges({athlete,attendance,anvil}){
  ...
  const anvilCount=(anvil||[]).filter(a=>a.athlete_name===athlete?.name).length;

  const BADGES=[
    {id:"first_early",icon:"🌅",label:"First Light",desc:"First early check-in",earned:early>=1,color:GREEN},
    {id:"streak_5",icon:"🔥",label:"On Fire",desc:"5-day streak",earned:streak>=5,color:RED},
    {id:"streak_10",icon:"💥",label:"Unstoppable",desc:"10-day streak",earned:streak>=10,color:RED},
    {id:"early_10",icon:"⚡",label:"Consistent",desc:"10 early check-ins",earned:early>=10,color:PUR},
    {id:"early_25",icon:"💪",label:"Iron Standard",desc:"25 early check-ins",earned:early>=25,color:PUR},
    {id:"anvil_1",icon:"⚒",label:"The Anvil",desc:"Won the Anvil award",earned:anvilCount>=1,color:GOLD},
    {id:"anvil_3",icon:"👑",label:"Anvil Legend",desc:"Won the Anvil 3 times",earned:anvilCount>=3,color:GOLD},
    {id:"faith",icon:"🙏",label:"Iron & Faith",desc:"Submitted a prayer request",earned:true,color:"#534AB7"},
  ];
  ...
  return(
    <div>
      <div style={{background:"#0f0f0f",borderRadius:12,padding:"1.25rem",marginBottom:12,border:"0.5px solid #222"}}>
        ...
        <div style={{fontSize:13,color:"#fff"}}>{earned.length} of {BADGES.length} badges earned</div>
      ...
      {locked.length>0&&(
        <div style={{background:"#fff",borderRadius:12,padding:"1.25rem",border:"0.5px solid #e0e0e0"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
            {locked.map((b,i)=>(
              <div key={i} style={{textAlign:"center",padding:"12px 8px",background:"#f9f9f9",borderRadius:10,border:"0.5px solid #e0e0e0",opacity:0.6}}>
      ...
      <>
        ...
        <div style={{minHeight:"100vh",background:"#f5f5f5",fontFamily:"Georgia, serif",maxWidth:480,margin:"0 auto"}}>
          ...
          <div style={{padding:"1.25rem"}}>
            ...
            {tab==="profile"&&(
              <div>
                {/* Class day reminder banner */}
                {(()=>{
                  const _d=new Date().getDay();
                  const isClassDay=[1,2,4,5].includes(_d);
                  if(!isClassDay)return null;
                  const isMonFri=_d===1||_d===5;
                  return(
                    <div style={{background:"linear-gradient(135deg,#C0392B,#8B1A1A)",borderRadius:12,padding:"12px 16px",marginBottom:12,border:"1px solid #C0392B44",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                      <div>
                        <div style={{fontSize:13,fontWeight:700,color:"#fff"}}>⚒ Class day — be early!</div>
                        <div style={{fontSize:11,color:"#ffaaaa"}}>Doors open at {isMonFri?"8:30am":"9:00am"} · On time is late</div>
                      </div>
                      <div style={{fontSize:24}}>🔥</div>
                    </div>
                  );
...
{tab==="polar"&&(
            ...
            {tab==="badges"&&<AchievementBadges athlete={selectedAthlete} attendance={attendance} anvil={anvils}/>}
