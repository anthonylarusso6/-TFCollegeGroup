  ...
  if(screen==="login")return(
    <>
      ...
      <div style={{minHeight:"100vh",background:"#080808",fontFamily:"Georgia, serif",maxWidth:480,margin:"0 auto",textAlign:"center",position:"relative",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem 1.5rem"}}>
        ...
        <div style={{fontSize:9,fontWeight:800,color:isForge?RED:STEEL,textTransform:"uppercase",letterSpacing:"0.22em",marginBottom:12,background:(isForge?RED:STEEL)+"15",padding:"3px 14px",borderRadius:20,border:"0.5px solid "+(isForge?RED:STEEL)+"33"}}>
        ...
        <div style={{position:"relative",marginBottom:16}}>
          ...
          <div style={{width:72,height:72,borderRadius:"50%",background:isForge?"linear-gradient(145deg,#E8720C,"+RED+",#8B0000)":"linear-gradient(145deg,#8a9aa4,"+STEEL+",#404a55)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,fontWeight:900,color:"#fff",overflow:"hidden",boxShadow:"0 0 40px "+(isForge?RED:STEEL)+"44"}}>
        ...
        <div style={{fontSize:22,fontWeight:900,color:"#fff",marginBottom:4,letterSpacing:"-0.02em",textTransform:"uppercase"}}>
        ...
        <div style={{display:"flex",justifyContent:"center",gap:14,marginBottom:16}}>
          {[0,1,2,3].map(i=>(
            <div key={i} style={{width:14,height:14,borderRadius:"50%",border:"2px solid "+(isForge?RED:STEEL)+(i<pin.length?"":"44"),background:i<pin.length?(isForge?"linear-gradient(135deg,#E8720C,"+RED+")":"linear-gradient(135deg,#8a9aa4,"+STEEL+")"):"transparent",transition:"all 0.15s",boxShadow:i<pin.length?"0 0 10px "+(isForge?RED:STEEL)+"88":"none"}}/>
          ))}
        </div>
        <input type="password" inputMode="numeric" maxLength={4} value={pin} onChange={e=>{const val=e.target.value.replace(/[^0-9]/g,"").slice(0,4);setPin(val);}} placeholder="····" style={{display:"block",width:130,margin:"0 auto 18px",padding:"10px",borderRadius:10,border:"1px solid #1e1e1e",background:"#0e0e0e",color:"#fff",fontSize:18,textAlign:"center",fontFamily:"Georgia,serif",letterSpacing:"0.3em",outline:"none",boxSizing:"border-box"}}/>
        {/* Keypad */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,maxWidth:240,margin:"0 auto"}}>
          {[1,2,3,4,5,6,7,8,9,null,0,"⌫"].map((k,i)=>(
            ...
            }} style={{padding:"14px 8px",borderRadius:12,border:"0.5px solid "+(k===null?"transparent":"#1a1a1a"),background:k===null?"transparent":k==="⌫"?"#141414":"#111",fontSize:20,fontWeight:300,cursor:k===null?"default":"pointer",color:k==="⌫"?"#555":"#e0e0e0",fontFamily:"sans-serif",transition:"background 0.1s",letterSpacing:0}}>
              {k===null?"":k}
            </button>
          ))}
        </div>
        {pinError&&<div style={{marginTop:14,fontSize:12,color:"#ff5555",padding:"8px 16px",background:"#1a0505",borderRadius:10,border:"1px solid #3a0808"}}>{pinError}</div>}
      </div>
    </>
  );
